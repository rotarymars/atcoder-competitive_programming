...
struct FastIO {
    static constexpr size_t IN_BUF_SIZE  = 1 << 20; // 1MB
    static constexpr size_t OUT_BUF_SIZE = 1 << 20; // 1MB
    ...
    // -------------------- low-level --------------------
    inline int gc() {  // ← char ではなく int を返す
        if (in_idx >= in_len) {
    ...
    inline bool skipSpaces(int &c) {
        ...
        } while (c <= ' '); // ここを使うなら <cctype> 不要 & unsigned キャスト不要
        return true;
    }
