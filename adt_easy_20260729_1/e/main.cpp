...
signed main() {
  ...
  sort(v.begin(),v.end());
  for(int i=1;i<=m;++i){
    auto it=lower_bound(v.begin(),v.end(),make_pair(i,make_pair(0,0))),itt=upper_bound(v.begin(),v.end(),make_pair(i+1,make_pair(0,0)));
    for(auto j=it;j!=itt;++j){
      ++cnt[j->second.second];
      --cnt[j->second.first];
      if(cnt[j->second.first]==0){
        cnt.erase(j->second.first);
      }
    }
    io.writeln(cnt.size());
  }
