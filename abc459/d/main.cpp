...
using namespace std;
#include <map>
bool isallfree(const vector<int>&cnt){
...
pair<char,char> maxchars(const vector<int>&cnt){
    pair<pair<char,int>,pair<char,int>>data{{'\0',-1},{'\0',-1}};
    for(int i=0;i<cnt.size();++i){
        if(data.first.first=='\0'&&cnt[i]!=0){
            data.first.first='a'+i;
            data.first.second=cnt[i];
        }
        else if(data.second.first=='\0'&&cnt[i]!=0){
            data.second.first='a'+i;
            data.second.second=cnt[i];
        }
        else{
            if(data.first.second>data.second.second)swap(data.first,data.second);
            if(data.second.second<cnt[i]&&cnt[i]!=0){
                data.first=data.second;
                data.second.first='a'+i;
                data.second.second=cnt[i];
            }
            else if(data.first.second<cnt[i]&&cnt[i]!=0){
                data.first.first='a'+i;
                data.first.second=cnt[i];
            }
        }
    }
    return {data.first.first,data.second.first};
}
signed main() {
  FastIO io;
  int t;
  io.read(t);
    while(t--){
        ...
        while(!isallfree(cnt)){
            // io.writeln(cnt);
            auto [a,b]=maxchars(cnt);
            // io.writeln(a,' ',b);
            if(ss.empty()){
                ss.push_back(b);
                --cnt[b-'a'];
            }
            else if(b!='\0'&&ss.back()!=b){
                ss.push_back(b);
                --cnt[b-'a'];
            }
            else if(a!='\0'&&ss.back()!=a){
                ss.push_back(a);
                --cnt[a-'a'];
            }
            else{
