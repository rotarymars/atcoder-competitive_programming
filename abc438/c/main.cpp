...
std::ostream &operator<<(std::ostream &os, const std::set<T> &s) {
  for (class std::set<T>::const_iterator i = s.begin(); i != s.end(); ++i) {
    os << *i;
    if (i != s.end() - 1) {
      os << " ";
    }
...
std::ostream &operator<<(std::ostream &os, const std::multiset<T> &s) {
  for (class std::multiset<T>::const_iterator i = s.begin(); i != s.end();
       ++i) {
    os << *i;
    if (i != s.end() - 1) {
      os << " ";
...
signed main() {
  ...
  io.read(n);
  list<int>v(n);
  for(auto i=v.begin();i!=v.end();++i)io.read(*i);
  for(auto i=v.begin();i!=v.end();++i){
    label:
    if(*i==*next(i)&&*next(i)==*next(next(i))&&*next(next(i))==*next(next(next(i)))){
      // io.writeln(distance(v.begin(),i), ' ',*i, ' ',*next(i), ' ',*next(next(i)), ' ',*next(next(next(i))));
      i=v.erase(i);
      i=v.erase(i);
      i=v.erase(i);
      i=v.erase(i);
