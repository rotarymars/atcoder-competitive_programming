...
signed main() {
  ...
  while (q--) {
    ...
    io.read(px, py, qx, qy, rx, ry, sx, sy);
    __int128 PX=px, PY=py, QX=qx, QY=qy, RX=rx, RY=ry, SX=sx, SY=sy;
    if((QX-PX)*(SY-RY)==(SX-RX)*(QY-RY)&&(QX-PX)*(PX+QX)*(SY-RY)+(PY+QY)*(QY-PY)*(SY-RY)!=(SX-RX)*(RX+SX)*(QY-PY)+(RY+SY)*(QY-PY)*(SY-RY)){
        io.writeln("No");
    }
    else{
        io.writeln("Yes");
