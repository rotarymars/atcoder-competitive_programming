...
template <class T> T COMBINATIONS(T a, T b, T mod) {
  if (b < 0 || b > a)
    return 0;
  T num = 1, den = 1;
  for (T i = 1; i <= b; ++i) {
    num = num * ((a + 1 - i) % mod) % mod;
    den = den * (i % mod) % mod;
  }
  T inv = 1;
  for (T e = mod - 2; e > 0; e >>= 1) {
    if (e & 1)
      inv = inv * den % mod;
    den = den * den % mod;
  }
...
signed main() {
  ...
  sum%=mod;
  long long COMBINATION_1=COMBINATIONS<long long>(n-1,k-1,mod),COMBINATION_2=COMBINATIONS<long long>(n-2,k-2,mod);
  for(int i=0;i<n;++i){
    long long base=(v[i]*v[i])%mod,m=COMBINATION_1;
    ans+=(base*m)%mod;
    // io.writeln(base, ' ', (base*m)%mod,' ',ans);
    ans%=mod;
  }
  for(int i=0;i<n;++i){
    long long tempsum=sum-v[i];
    while(tempsum<0)tempsum+=mod;
    ans+=v[i]%mod*tempsum%mod*COMBINATION_2%mod;
    ans%=mod;
    // io.writeln(tempsum, ' ', (v[i]*tempsum*(COMBINATIONS(n-2,k-2)%mod))%mod, ' ', ans);
  }
  io.writeln(ans);
  return 0;
}
