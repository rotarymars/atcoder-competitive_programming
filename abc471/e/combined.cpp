...
signed main() {
  ...
  sum %= mod;
  long long COMBINATION_1 = COMBINATIONS<long long>(n - 1, k - 1, mod),
            COMBINATION_2 = COMBINATIONS<long long>(n - 2, k - 2, mod);
  for (int i = 0; i < n; ++i) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    ans %= mod;
    // io.writeln(tempsum, ' ', (v[i]*tempsum*(COMBINATIONS(n-2,k-2)%mod))%mod,
    // ' ', ans);
  }
  io.writeln(ans);
  return 0;
