...
signed main() {
  io.read(n, k);
  reach.assign(n + 2, vector<bool>(1 + k, false));
  reach[n + 1][0] = true;
  for (int i = n; i >= 1; --i)
    for (int r = 0; r <= k; ++r)
      reach[i][r] = reach[i + 1][r] || (r >= i && reach[i][r - i]);
  jump.assign(n + 1, vector<int>(1 + k, INT_MAX));
  for (int i = 1; i <= n; ++i)
    for (int r = 0; r <= k; ++r) {
      if (reach[i + 1][r])
        jump[i][r] = 0;
      else if (r >= i && jump[i][r - i] != INT_MAX)
        jump[i][r] = jump[i][r - i] + 1;
    }
  a.assign(n + 1, 0);
  f(1, k);
  return 0;
